...
# TODO Решение задач Задача 1. Напишите функцию, которая возвращает длину гипотенузы
# прямоугольного треугольника по известным значениям его катетов.
# Решение. Для нахождения длины гипотенузы, нам нужно применить теорему Пифагора:
...
# Напомним, что результатом обоих выражений: math.sqrt(c) и c ** 0.5 является одно число
...
def get_shipping_cost(quantity):
    ...
    if quantity == 1:
        return price
    return price + next_price * (quantity - 1)
...
#TODO Биномиальный коэффициент 🌶️
...
def number_to_words(num):
    months = {1: 'один', 2: 'два', 3: 'три', 4: 'четыре', 5: 'пять', 6: 'шесть',
    ...
    if 1 <= num < 20 or num % 10 == 0:
        return months[num]
    return f"{months[num - num % 10]} {months[num % 10]}"
...
def number_to_words(num):
    nums = ['один', 'два', 'три', 'четыре', 'пять', 'шесть', 'семь', 'восемь',
    ...
    if num <= 20:
        return nums[num - 1]
    return f"{nums[(num // 10) + 17]} {nums[(num % 10) - 1]}"
...
def get_month(language, number):
    months = {
            "ru": {1: 'январь', 2: 'февраль', 3: 'март', 4: 'апрель', 5: 'май',
...
#представление корректой даты и возвращает значение True если дата является 
#магической и False в противном случае.
